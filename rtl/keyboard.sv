...
wire       shift       = shiftExtra & ((shiftL & shiftR) ^ capsLock);
reg  [4:0] keys[14:0];
reg        capsLock;
reg        shiftL;
reg        shiftR;
...
reg        stop;       // ctrlL and ctrlR acts as stop key too
assign     columns     = { (stop & ctrlL & ctrlR), shift, keys[row] };
...
begin
   ...
   begin
      ...
      begin
         /* Extended keys */
         case(code)
            8'h7d : keys[14][0] <= ~pressed; // (R)CL = Page Up (E07D)
            8'h70 : keys[12][1] <= ~pressed; // INS (e070)   
            8'h71 : keys[13][1] <= ~pressed; // DEL = Delete (E071)
            8'h7a : keys[14][1] <= ~pressed; // CLR = Page Down (E07A)
            ...
            8'h11 : altR        <= ~pressed; // Right alt
            8'h14 : ctrlR       <= ~pressed; // Right Ctrl (E014)
         endcase
      end
      else
      begin
         ...
         case(code)
            ...
            8'h11 : altL        <= ~pressed; // Left alt
            8'h14 : ctrlL       <= ~pressed; // Left Ctrl
            ...
            //8'h7d : keys[14][0] <= ~pressed; // RCL = Page UP
            ...
            8'h66 : keys[12][2] <= ~pressed; // <--- (sipka vlevo) = Backspace ... another mapping to <---
